...
void ResponseMediator::notify(const std::string& sender,
                              const std::string& status)
{
    std::cout << "[Mediator] " << sender
              << " reported status: " << status << std::endl;

    if (sender == "SecurityTeam" && status == "READY")
    {
        std::cout << "[Mediator] Security is ready. "
                  << "Informing Medical Team and Facilities Staff."
                  << std::endl;

        if (medicalTeam != 0)
        {
            medicalTeam->receiveAlert(
                "Security team is ready for medical coordination.");
        }

        if (facilitiesStaff != 0)
        {
            facilitiesStaff->receiveAlert(
                "Security team is ready. Prepare facilities.");
        }
    }
}